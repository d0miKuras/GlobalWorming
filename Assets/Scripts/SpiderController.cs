...
public class SpiderController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        float valueY = _agent.velocity.y;
        if (valueY != 0)
            // transform.position += transform.forward * valueY * _speed * multiplier * Time.fixedDeltaTime;
            transform.position += transform.forward * valueY * _agent.speed * Time.fixedDeltaTime;

        // float valueX = Input.GetAxis("Horizontal");
        float valueX = _agent.velocity.x;
        if (valueX != 0)
            // transform.position += Vector3.Cross(transform.up, transform.forward) * valueX * _speed * multiplier * Time.fixedDeltaTime;
             transform.position += Vector3.Cross(transform.up, transform.forward) * valueX * _agent.speed * Time.fixedDeltaTime;
